...
public:
  ...
  void SetRegType(int val)
  {
    fRegType   = val;
  }
  void SetRegWeight(double val)
  {
    fRegWeight = val;
  }
  void SetLMatrix(TMatrixD &L)
  {
    fMatL.ResizeTo(L);
    fMatL = L;
  }
  void SetPrior(TH1D *h);
  void SetSmoothingWeights(TVectorD &wts)
  {
    fSmoothingWeight = wts;
  }

  Double_t GetTrueX1()      const
  {
    return fTrueX1;
  }
  Double_t GetTrueX2()      const
  {
    return fTrueX2;
  }
  Double_t GetMeasX1()      const
  {
    return fMeasX1;
  }
  Double_t GetMeasX2()      const
  {
    return fMeasX2;
  }
  ...
  Int_t GetM()              const
  {
    return fM; // Number of measured bins
  }
  Int_t GetNMeasBins()      const
  {
    return fM; // Number of measured bins
  }
  Int_t GetN()              const
  {
    return fN; // Number of true/generated bins
  }
  Int_t GetNTrueBins()      const
  {
    return fN; // Number of true/generated bins
  }
  TMatrixD GetA(TString opt = "");                     // "^", "~", or ""
  TVectorD Getb(TString opt = "");                     // "~" or ""
  TMatrixD GetbCovariance() const
  {
    return fMatB; // Error matrix of b
  }
  TMatrixD GetbBinv()       const
  {
    return fMatBinv; // Inverse error matrix of b
  }
  TMatrixD GetLMatrix()     const
  {
    return fMatL;
  }
  TVectorD GetxTrue()       const
  {
    return fVecXtrue; // b, scaled by its error
  }
  TH2D *GetAProbHist()      const
  {
    return fHistAProb;
  }
  TH2D *GetAHist()          const
  ...
  TH2D *GetATildeHist()     const
  {
    return fHistATilde;
  }
  TH2D *GetBCovHist()       const
  {
    return fHistMeasCov;
  }
  TH1D *GetbTildeHist()     const
  {
    return fHistbTilde;
  }
  TH1D *GetXiniHist()       const
  {
    return fHistXini;
  }
  TH1D *GetXTrueHist();
  TH1D *GetEffHist()        const
  {
    return fHistEff;
  }
  TH1D *GetPrior()          const
