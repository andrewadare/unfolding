...
void RUExample()
{
  ...
  DrawObject(xini, "", "ru_xini_bini", cList);
  bini->SetLineColor(kBlue);
  bini->Draw("same");
  DrawObject(datatrue, "", "ru_data", cList);
  data->Draw("same");

  // Response matrix, as generated (counts)
  DrawObject(Adet,"colz","ru_A", cList);
  ...
  hAprob->SetTitle("Detector response (normalized to efficiency);"
                   "measured;true");
  DrawObject(hAprob,"colz","ru_Ahat", cList);
  ...
  if (printPDFs)
  {
    ...
    PrintPDF(cList, "pdfs/ru_example_plots");
  }
